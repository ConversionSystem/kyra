"""
KLAW Router Core — Standalone K-104 intelligent model routing.
    ...
    def _load(self):
        try:
            with open(self.path, "rb") as f:
                magic = f.read(4)
                if magic != b"CLAW":
                    return  # Not a valid .claw file — skip gracefully
            ...
            # rooms.claw decryption failed or unavailable — router works without it
            # Kyra uses its own template system via KYRA_TEMPLATE_PATTERNS
            self._data = None
...
class KClassifier:
    ...
    KYRA_TEMPLATE_PATTERNS = [
        # Hours / availability
        (r"what (are|'re) (your|the) (hours|opening hours|business hours)", "hours_query"),
        (r"(are you|are you guys) open", "hours_query"),
        (r"when (do you|are you) (open|close|available)", "hours_query"),
        # Booking / appointments
        (r"(can i|how do i|i want to) (book|schedule|make) (an? )?(appointment|booking|visit|call|demo)", "booking_query"),
        (r"(book|schedule) (me|an? appointment|a call)", "booking_query"),
        (r"(is|are) (there )?(any )?(appointments?|slots?|availability) (available|open|free)", "booking_query"),
        # Pricing
        (r"(how much|what'?s? the (cost|price|pricing)|what do you charge)", "pricing_query"),
        (r"(do you have|what are) (your )?(prices?|rates?|fees?|plans?)", "pricing_query"),
        # Location
        (r"(where are you|what'?s? your address|where is)", "location_query"),
        (r"(do you have|are there) (a )?(location|locations|office|offices|store)", "location_query"),
        # Contact
        (r"(what'?s? your (phone|email|number)|how (can|do) i (contact|reach|get in touch))", "contact_query"),
        # Services
        (r"(what (services|do you offer|do you do)|what'?s? included)", "services_query"),
        # Simple yes/no
        (r"^(yes|no|ok|okay|sure|got it|sounds good|perfect|great|awesome|cool)\.?$", "acknowledgment"),
        (r"^(thank you|thanks|thx|ty)\.?$", "thanks"),
        (r"^(no thanks|not right now|not interested|maybe later)\.?$", "decline"),
    ]
    ...
    def classify(self, query: str) -> dict:
        ...
        # Kyra: check business template patterns first (tier 0 — free)
        for pattern, reason in self.KYRA_TEMPLATE_PATTERNS:
...
class KlawRouter:
    ...
    def __init__(self, api_keys: dict = None, daily_cap: float = 5.00,
                 ...
                 skip_license: bool = True):   # Kyra: skip_license=True by default (MIT)
