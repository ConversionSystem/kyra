/**
 * NAP Auditor — Executable skill for the Cannabis SEO Worker
 *
 * Scrapes 20 cannabis directories and compares Name, Address, Phone
 * data against the dispensary's master record. Flags inconsistencies.
 */

import directoriesConfig from '../../config/dispensary-directories.json';
...
function normalizeName(name: string): string {
  return name
    ...
    // Strip cannabis-specific suffixes that commonly vary across directories
    .replace(/\b(cannabis co|cannabis company|cannabis|dispensary|weed|herb|bud)\b/gi, '')
    .replace(/[.,'"]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function compareName(master: string, found: string): { match: boolean; issue: string | null } {
  ...
  // Handle common cannabis naming variations:
  // "dispensary" ↔ "canna" ↔ "weed" ↔ "herb" ↔ "bud"
  // "delivery" ↔ "delivers"
  const cannabisVariations = [
    [/\bdispensary\b/, 'canna'],
    [/\bcanna\b/, 'dispensary'],
    [/\bweed\b/, 'cannabis'],
    [/\bcannabis\b/, 'weed'],
    [/\bherb\b/, 'bud'],
    [/\bbud\b/, 'herb'],
    [/\bdelivery\b/, 'delivers'],
    [/\bdelivers\b/, 'delivery'],
  ] as const;

  let expandedMaster = normMaster;
  for (const [pattern, replacement] of cannabisVariations) {
...
function extractNAPFromContent(
  ...
  const lowerDispensary = dispensaryName.toLowerCase();

  // Check if dispensary is mentioned at all
  if (!lowerContent.includes(lowerDispensary) &&
      levenshtein(lowerDispensary, lowerContent.slice(0, 200)) > 10) {
    return null;
  }

  const result: Partial<MasterNAP> = {};

  // Extract name — look for the dispensary name in headers or bold text
  const namePattern = new RegExp(
    `(?:^|\\n|\\*\\*|##?\\s*)(${dispensaryName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^\\n]*?)(?:\\n|\\*\\*|$)`,
    'im',
  );
  ...
  const dispensaryUrl = urls.find(
    (u) => !u.includes('weedmaps.com') && !u.includes('leafly.com') &&
           !u.includes('yelp.com') && !u.includes('facebook.com') &&
           !u.includes('google.com') && !u.includes('iheartjane.com') &&
           !u.includes('dutchie.com'),
  );
  if (dispensaryUrl) result.website = dispensaryUrl;

  // Extract state cannabis license — common formats: C10-0000123-LIC, CDPH-10000001, etc.
  const licensePattern = /\b(C\d{1,3}-\d{7,}-LIC|CDPH-\d+|A-\d{6,}|M-\d{6,}|[A-Z]{2,4}-?\d{6,})\b/;
  const licenseMatch = content.match(licensePattern);
  if (licenseMatch) result.license_number = licenseMatch[1];
...
export async function runNAPAudit(
  ...
  for (const dir of directories) {
    const searchUrl = dir.url_pattern
      .replace(/\{\{DISPENSARY_NAME\}\}/g, encodeURIComponent(masterNAP.name))
      .replace(/\{\{CITY\}\}/g, encodeURIComponent(dispensaryCity));
    ...
    try {
      ...
      if (!scraped || !scraped.content) {
        ...
        actionItems.push(`Claim ${dir.name} listing — dispensary not found`);
      } else {
        ...
        if (!foundData) {
          ...
          actionItems.push(`Claim ${dir.name} listing — dispensary not found in search results`);
        } else {
          const issues: string[] = [];
          ...
          // Cannabis-specific: state license number check
          if (masterNAP.license_number && foundData.license_number &&
              masterNAP.license_number !== foundData.license_number) {
            issues.push(`License Number: '${foundData.license_number}' vs '${masterNAP.license_number}'`);
          }
