...
// ============ Container Tools MD Builder ============

/**
 * Build the TOOLS.md content that gets written to every container's workspace.
 * This tells the OpenClaw AI what tools are available via Kyra's API bridge:
 * - GHL tools (book appointments, tag contacts, create deals, send messages)
 * - CRM context (look up contact history, deals, relationship memory)
 *
 * The container uses its gateway_token as the Bearer auth for these calls.
 * Kyra resolves which client the token belongs to and executes accordingly.
 */
function buildContainerToolsMd(appUrl: string): string {
  return `# TOOLS.md — Kyra Platform Tools

You have access to these external tools via HTTP calls. Use them when appropriate.
Your auth token (gateway_token) is your identity — include it as Bearer in all calls.

---

## GHL Tools (GoHighLevel CRM)

**Endpoint:** POST ${appUrl}/api/agent/ghl-tool
**Auth:** Bearer {your_gateway_token}
**Body:** { "tool": "<tool_name>", "args": { ... }, "contact_id": "<optional>" }

Available tools:

### Contacts
- \`create_contact\` → { first_name, last_name, email, phone, company_name, tags }
- \`update_contact\` → { contact_id, first_name, last_name, email, phone }
- \`search_contacts\` → { query }
- \`add_contact_tags\` → { contact_id, tags: string[] }
- \`remove_contact_tags\` → { contact_id, tags: string[] }
- \`add_contact_note\` → { contact_id, body }

### Calendar
- \`book_appointment\` → { contact_id, calendar_id, start_time, end_time, title }
- \`get_available_slots\` → { calendar_id, start_date, end_date }
- \`get_calendars\` → {}
- \`reschedule_appointment\` → { appointment_id, start_time, end_time }
- \`cancel_appointment\` → { appointment_id }

### Opportunities (Deals)
- \`create_opportunity\` → { contact_id, name, pipeline_id, stage, value }
- \`update_opportunity\` → { opportunity_id, stage, value, name }
- \`move_pipeline_stage\` → { opportunity_id, stage }
- \`mark_won_lost\` → { opportunity_id, status: "won"|"lost" }

### Conversations
- \`send_sms\` → { contact_id, message }
- \`send_email\` → { contact_id, subject, body }
- \`send_whatsapp\` → { contact_id, message }

### Tasks
- \`create_task\` → { contact_id, title, due_date, priority }
- \`complete_task\` → { task_id }

### Invoices
- \`create_invoice\` → { contact_id, items: [{name, price, quantity}] }
...
**Endpoint:** POST ${appUrl}/api/agent/crm-context
**Auth:** Bearer {your_gateway_token}
**Body:** { "phone": "...", "email": "...", "contact_id": "...", "query": "..." }

Use this to look up a contact's full history, deals, tasks, and relationship memories.
Pass \`query\` without phone/email/contact_id to get aggregated CRM stats.

**Returns:**
- Contact name, stage, lead score, tags
- Open deals with values
- Pending tasks
- Recent activity (calls, emails, SMS)
- Relationship memory (personal details, preferences, objections)

---

## When to Use These Tools

- Customer asks to book an appointment → use \`book_appointment\` or \`get_available_slots\`
- New lead reaches out → use \`create_contact\` + \`create_opportunity\`
- Customer shows buying intent → use \`add_contact_tags\` (e.g., "hot-lead")
- "Who are our hot leads?" → use CRM context with query
- Customer is upset / needs human → use \`add_contact_note\` to log it
- "What deals do we have open?" → use CRM context with query
- Send follow-up SMS → use \`send_sms\`

Always confirm actions with the user before executing writes (create, update, delete).
Reads (search, get, context lookup) are safe to do proactively.

---

*This file is auto-generated by Kyra. Do not edit manually.*
...
export async function provisionClientGateway(
  ...
  try {
    ...
    // Build TOOLS.md that tells the container about GHL + CRM API bridges
    // This is what makes "OpenClaw is the brain" true — the container knows
    // it can call back to Kyra to execute GHL actions and look up CRM data.
    const appUrl = (process.env.NEXT_PUBLIC_APP_URL || 'https://kyra.conversionsystem.com').replace(/\/+$/, '');
    const toolsMd = config.toolsMd || buildContainerToolsMd(appUrl);
    ...
    const res = await provisionerFetch('/containers', {
      ...
      body: JSON.stringify({
        ...
        firecrawlEnv: {
          FIRECRAWL_API_URL: `${appUrl}/api/fc`,
          FIRECRAWL_API_KEY: `kyra-agency-${agencyId}`,
        },
        // Kyra API bridge URLs — injected as env vars on the container
        // These allow the container to call back to Kyra's API
        kyraEnv: {
          KYRA_API_URL: appUrl,
          KYRA_GHL_TOOL_URL: `${appUrl}/api/agent/ghl-tool`,
          KYRA_CRM_CONTEXT_URL: `${appUrl}/api/agent/crm-context`,
        },
      }),
    });
