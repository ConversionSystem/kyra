...
export function transparentOverlayNavbar(data: NavbarData): string {
  ...
  const cta = data.bookingUrl
    ? `<a href="${data.bookingUrl}" class="px-5 py-2 rounded-lg text-sm font-semibold transition-opacity hover:opacity-90" style="background: var(--color-accent); color: var(--color-surface);">Book Now</a>`
    : data.phone
      ? `<a href="${data.phoneHref || `tel:${data.phone}`}" class="px-5 py-2 rounded-lg text-sm font-semibold transition-opacity hover:opacity-90" style="background: var(--color-accent); color: var(--color-surface);">${data.phone}</a>`
      : '';
