...
export function statsBarAbout(data: AboutData): string {
  ...
  while (stats.length < 4) {
    stats.push({ value: '100%', label: 'Satisfaction', icon: `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.8)" stroke-width="1.8"><polyline points="20 6 9 17 4 12"></polyline></svg>` });
  }

  const cells = stats.slice(0, 4).map((s, i) => {
    const isLast = i === stats.length - 1 || i === 3;
