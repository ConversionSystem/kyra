...
export function twoColumnFaq(data: FaqData): string {
  ...
  const cards = data.faqs.map(faq => `
    <div class="rounded-xl shadow-md p-6" style="background: var(--color-surface);">
  ...
  return `<section class="py-16 sm:py-24 px-4" style="background: var(--color-surface);" aria-label="FAQ">
