...
export function gridCardsTestimonials(data: TestimonialsData): string {
  ...
  const cards = data.testimonials.map(t => `
    <div class="rounded-xl p-6 shadow-md" style="background: var(--color-surface); border: 1px solid var(--color-border);">
      <div class="text-4xl mb-3" style="color: var(--color-primary);">&ldquo;</div>
