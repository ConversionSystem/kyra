...
export function alternatingServices(data: ServicesData): string {
  ...
  const rows = data.services.map((s, i) => {
    ...
    const desc = s.description
      ? `<p style="color: #6b7280; font-size: 1rem; line-height: 1.8; margin: 0 0 1.5rem 0;">${s.description}</p>`
      : '';
    ...
    const textBlock = `<div class="flex flex-col justify-center">
          ...
          ${desc}
          <a href="#${s.slug}" style="color: ${primary}; font-weight: 600; font-size: 0.938rem; text-decoration: none;">Learn more &rarr;</a>
        </div>`;
