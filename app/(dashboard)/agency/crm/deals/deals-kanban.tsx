...
import {
  ...
  Trash2, ArrowRight, Sparkles, Target, Save, Clock, FileText,
  Edit2, ExternalLink,
} from 'lucide-react';
      ...
      {selectedDeal && (
        <DealSlideOver
          ...
          onUpdate={async (updates) => {
            await fetch(`/api/agency/crm/deals/${selectedDeal.id}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
          ...
          onDelete={async () => {
            await fetch(`/api/agency/crm/deals/${selectedDeal.id}`, { method: 'DELETE' });
...
function DealSlideOver({ deal, onClose, onUpdate, onDelete }: {
  deal: Deal;
  onClose: () => void;
        ...
        <div className="p-4 sm:p-6 space-y-6">
          ...
          {editing ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    type="number" min="0" max="100" value={form.probability} onChange={e => setForm(f => ({ ...f, probability: e.target.value }))} />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Close Date</label>
                  <input className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              ...
              <div className="flex gap-2">
                <Button className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white" disabled={saving} onClick={handleSave}>
            ...
            <>
              <div className="flex items-baseline gap-3">
                <p className="text-4xl font-bold text-gray-900">${Number(deal.value).toLocaleString()}</p>
                <span className="text-sm text-gray-500 capitalize px-2 py-0.5 bg-gray-100 rounded-full">{deal.stage}</span>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 text-sm">
              ...
              {deal.close_date && (
                <>
                  ...
                  <span className="text-gray-900">{new Date(deal.close_date).toLocaleDateString()}</span>
                </>
              )}

              <span className="text-gray-500">Source</span>
              <span className="text-gray-900 capitalize">{deal.source || 'Manual'}</span>

              <span className="text-gray-500">Created</span>
              <span className="text-gray-900">{new Date(deal.created_at).toLocaleDateString()}</span>

              <span className="text-gray-500">Weighted Value</span>
              <span className="text-gray-900 font-medium">${Math.round(Number(deal.value) * deal.probability / 100).toLocaleString()}</span>
            </div>
          </div>
          ...
          {(contactName || deal.company) && (
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              <h3 className="text-xs font-bold text-gray-900 uppercase tracking-wide">Linked To</h3>
              {contactName && (
                <div className="flex items-center gap-2 text-sm">
                  <User className="h-4 w-4 text-indigo-500" />
                  <span className="text-gray-900">{contactName}</span>
                  {deal.contact?.email && (
                    <span className="text-xs text-gray-400">{deal.contact.email}</span>
                  )}
                </div>
              )}
              {deal.company && (
          ...
          {/* Notes */}
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-gray-900 uppercase tracking-wide flex items-center gap-1">
              <FileText className="h-3.5 w-3.5" /> Notes
            </h3>
            {deal.notes && (
              <p className="text-sm text-gray-700 whitespace-pre-wrap bg-gray-50 rounded-lg p-3">{deal.notes}</p>
            )}
            <div className="flex gap-2">
              <input
              ...
              />
              <Button size="sm" variant="outline" disabled={!noteText.trim()} onClick={handleAddNote}>
                Add
              </Button>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-gray-200">
            <Button
              ...
              onClick={async () => {
                if (confirm('Delete this deal? This cannot be undone.')) await onDelete();
              }}
            >
